...
def configure_logging(fpath):
    ...
    # 로그 파일 압축 설정
    logzip_handler = logging.handlers.TimedRotatingFileHandler(
        "run.log", when="d", backupCount=10, encoding='utf-8', delay=True
    )
    logzip_handler.setLevel(logging.ERROR)
    logzip_handler.setFormatter(logging.Formatter(log_format, date_format, style='{'))
    logzip_handler.suffix = "%Y%m%d.zip"
    logzip_handler.extMatch = r"^[\d]{8}.zip$"
    ...
    logger.addHandler(logzip_handler)
    return logger
